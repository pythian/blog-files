...
--
--
-- Usage:  AWR_load_trends.sql, and provide the input parameters as illustrated below
--
--
-- Example:
-- SQL> @AWR_load_trends.sql
-- 
-- 
-- Instances in this Workload Repository schema
-- ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
-- 
-- IS    DB Id    Inst Num DB Name      Instance     Host
-- -- ----------- -------- ------------ ------------ ------------
...
select distinct
  ...
  order by wr.instance_name, wr.instance_number, wr.dbid;
...
with snaps as
 ( /* dba_hist_snapshot */
    select begin_snap_id,
           ...
           end_snap_time,
           instance_number,            
           86400*(cast(end_snap_time as date) - cast(begin_snap_time as date)) elapsed,
           startup_time, 
           p_startup_time 
    from (      
      select dbid, 
             ...
             lag(end_interval_time) over (partition by dbid, instance_number order by dbid, instance_number, snap_id) begin_snap_time,                                                              
       ...
       where dbid = to_number('&p_dbid')
         ...
         and end_interval_time between to_timestamp('&start_date_time','DD-MON-YYYY HH24:MI') and to_timestamp('&end_date_time','DD-MON-YYYY HH24:MI')           
    )        
 )
select  snaps.instance_number, 
 ...
 ( /* dba_hist_osstat */
     select *
       from       
       (select s.instance_number,s.snap_id,s.dbid,s.stat_name,s.value 
          from dba_hist_osstat s, snaps
         where s.stat_name in ('NUM_CPUS',
                               'NUM_CPU_CORES',
                               'BUSY_TIME',
                               'LOAD',
                               'USER_TIME',
  ...
  ( /* dba_hist_sys_time_model */
     select * from
          (select m.instance_number,m.snap_id,m.dbid,m.stat_name,m.value 
             from dba_hist_sys_time_model m, snaps
            where m.stat_name in ('DB time',
                                  'DB CPU',
                                  'background cpu time',
  ...
  ( /* dba_hist_sysstat */
    select * from
        (select s.instance_number, s.snap_id, s.dbid, s.stat_name, s.value 
           from dba_hist_sysstat s, snaps
           where s.stat_name in ( 'logons current',                        
                                  'logons cumulative',                     
                                  'execute count',                         
